...
import { Avatar, Popover, MenuList, MenuItem, Paper, ClickAwayListener } from "@material-ui/core";
...
import EditMusicModal from "../EditMusicModal/EditMusicModal";
...
function FooterMusicPlayer({ music }) {
    ...
    const [editModalOpen, setEditModalOpen] = useState(false); // Nuevo estado para controlar la apertura del modal de edición de música
    const [anchorEl, setAnchorEl] = useState(null); // Estado para el elemento ancla del menú emergente
    ...
    const isDesktop = useMediaQuery("(min-width: 431px)"); // Media query para escritorio y tabletas
    ...
    const handleMenuOpen = (event) => {
        setAnchorEl(event.currentTarget); // Establece el elemento ancla del menú emergente
    };

    const handleMenuClose = () => {
        setAnchorEl(null); // Cierra el menú emergente
    };

    const handleEditMusic = () => {
        setEditModalOpen(true); // Abre el modal de edición de música
        handleMenuClose(); // Cierra el menú emergente después de hacer clic en "Editar música"
    };

    const handleAddToPlaylist = () => {
        setPlaylistModalOpen(true); // Abre el modal de lista de reproducción
        handleMenuClose(); // Cierra el menú emergente después de hacer clic en "Agregar a la playlist"
    };
            ...
            <div className="playback-controls">
                ...
                <Popover
                    ...
                    transformOrigin={{
                        ...
                        vertical: isDesktop ? 'top' : 'bottom', // Usa 'top' solo en escritorio y tabletas
                        horizontal: 'center',
                    }}
                    style={{ marginTop: isDesktop ? "-70px" : "-75px" }} // Ajusta el margen superior solo en escritorio y tabletas
                >
                    <Paper>
                        <ClickAwayListener onClickAway={handleMenuClose}>
                            <MenuList>
                                <MenuItem onClick={handleEditMusic}>Editar música</MenuItem>
                                <MenuItem onClick={handleAddToPlaylist}>Agregar a la playlist</MenuItem>
                            </MenuList>
                        </ClickAwayListener>
                    </Paper>
                </Popover>
                <PlaylistModal open={playlistModalOpen} onClose={() => setPlaylistModalOpen(false)} />
                <EditMusicModal open={editModalOpen} onClose={() => setEditModalOpen(false)} music={music} />
            </div>
